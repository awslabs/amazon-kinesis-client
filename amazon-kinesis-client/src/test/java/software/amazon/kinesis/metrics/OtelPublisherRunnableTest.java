...
public class OtelPublisherRunnableTest {
    ...
    private static class TestHarness {
        private List<MetricDatumWithKey<OtelMetricKey>> data = new ArrayList<MetricDatumWithKey<OtelMetricKey>>();
        private int counter = 0;
        private OtelMetricsPublisher publisher;
        private OtelPublisherRunnable runnable;
        private long time = 0L;

        TestHarness() {
            publisher = Mockito.mock(OtelMetricsPublisher.class);
            runnable = new OtelPublisherRunnable(publisher, MAX_BUFFER_TIME_MILLIS, MAX_QUEUE_SIZE, FLUSH_SIZE) {
        ...
        private MetricDatumWithKey<OtelMetricKey> constructDatum(int value) {
            MetricDatum datum = TestHelper.constructDatum(
                    "datum-" + Integer.toString(value), StandardUnit.COUNT, value, value, value, 1);

            return new MetricDatumWithKey<OtelMetricKey>(new OtelMetricKey(datum), datum);
        }

        /**
         * Run one iteration of the runnable and assert that it called the publisher with count records beginning with
        ...
        /**
         * Run one iteration of the runnable and assert that it called the publisher with all data.
         */
        ...
        OtelPublisherRunnable getRunnable() {
            return runnable;
        }
    }
    ...
    /**
     * Enqueue FLUSH_SIZE + 1 messages. Shutdown the runnable. Without passing time, assert that the runnable sends all
